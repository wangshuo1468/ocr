...
public class OcrUtils {
    ...
    public static String ocrImg(byte[] imgData) {
        String s = bdGeneralOcr(imgData);
        String s1 = bdAccurateOcr(imgData);
        String s2 = sogouMobileOcr(imgData);
        s = s.replaceAll("\\s*", "");
        s1 = s1.replaceAll("\\s*", "");
        s2 = s2.replaceAll("\\s*", "");
        if(  s.length() >  s1.length() && s.length() >   s2.length()){
            return  s;
        }else if(s1.length() >  s.length()  && s1.length()  > s2.length()){
            return  s1;
        }else if(s2.length() >  s.length()  && s2.length()  > s1.length()){
            return  s2;
        }else {
            return  null;
        }
